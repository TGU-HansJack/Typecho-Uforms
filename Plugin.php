...
class Uforms_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function activate()
    {
        try {
            ...
            Helper::addPanel(2, 'Uforms/admin/ajax.php', 'Uforms Ajax', 'Uforms Ajax处理', 'administrator');
    ...
    public static function deactivate()
    {
        ...
        Helper::removePanel(2, 'Uforms/admin/ajax.php');
    ...
    private static function createTables()
    {
        ...
        $sql = "CREATE TABLE IF NOT EXISTS `{$prefix}uforms_notifications` (
            ...
            `is_read` tinyint(1) DEFAULT 0 COMMENT '是否已读',  -- 添加缺失的字段
            `read_time` int(11) DEFAULT 0 COMMENT '阅读时间',   -- 添加缺失的字段
            PRIMARY KEY (`id`),
            KEY `form_id` (`form_id`),
            KEY `submission_id` (`submission_id`),
            KEY `status` (`status`)
        ...
        $sql = "CREATE TABLE IF NOT EXISTS `{$prefix}uforms_calendar` (
            ...
            `event_description` text COMMENT '描述', -- 修复：避免使用description关键字
            `author_id` int(11) NOT NULL COMMENT '创建者ID',
...
public static function adminHeader()
{
    ...
    if ($request->panel === 'Uforms/index.php') {
